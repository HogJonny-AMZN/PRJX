...
IF EXIST "%HOG_PY_BASE_VENV_EXE%" (
    GOTO install_hog_py_shared_packages_standard
) ELSE (
   GOTO shared_standard_packages_failure
)

:shared_standard_packages_failure
...
:install_hog_py_shared_packages_standard
REM install the requirements\requirements.txt into bootstrap 3P site-packages
echo Calling PIP to install "%HOG_PY_REQUIREMENTS_3P%" into bootstrap `shared` 3P site-packages ...
echo    Into: %HOG_PY_SHARED_3P_SITE%
echo.
call "%HOG_PY_BASE_VENV_PIP%" install -r "%HOG_PY_REQUIREMENTS_3P%" --target="%HOG_PY_SHARED_3P_SITE%" --disable-pip-version-check --no-warn-script-location
if ERRORLEVEL 1 (
    echo Failed installing requirements into 3P Python bootstrap `shared` site-packages ...
    echo Requirements packages could not be installed: %HOG_PY_REQUIREMENTS_3P%
    echo Failed to install the packages listed in: %HOG_PY_SHARED_3P_SITE%.  Check the log above!
    EXIT /b 1
)
